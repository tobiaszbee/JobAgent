...
function render() {
  ...
  if (sort === 'date')    jobs.sort((a, b) => new Date(b.found_at) - new Date(a.found_at));
    ...
    <div class="job-card status-${esc(j.status)}" id="card-${esc(j.id)}">
      <div class="job-header">
        <div class="job-info">
          ...
          <div class="job-sub">${esc(j.company)} &nbsp;·&nbsp; 📍 ${esc(j.location)}</div>
        </div>
        <div class="score-badge ${sc}">${score}</div>
      </div>
      ${j.reasoning ? `<div class="job-reasoning">${esc(j.reasoning)}</div>` : ''}
      <div class="job-footer">
        <span class="status-tag tag-${esc(j.status)}">${esc(j.status)}</span>
        <span class="job-date">${formatDate(j.found_at)}</span>
        <div class="actions">
...
async function loadCriteria() {
  ...
  grid.innerHTML = Object.entries(CRITERIA_LABELS).map(([type, label]) => {
    ...
    const allActive = items.every(c => c.active);
    return `
      <div class="criteria-group" data-type="${type}">
        ...
        ${items.map(c => `
          <div class="criteria-item" id="ci-${c.id}">
            <input type="checkbox" id="cb-${c.id}" ${c.active ? 'checked' : ''}
              onchange="toggleCriteria(${c.id}, this.checked)">
            <label for="cb-${c.id}" class="${c.active ? '' : 'inactive'}">${c.value}</label>
...
async function openRunModal() {
  ...
  const titles    = all.filter(c => c.type === 'title'    && c.active);
  const locations = all.filter(c => c.type === 'location' && c.active);

  document.getElementById('run-titles').innerHTML = titles.map(c => `
    <label class="run-check">
      <input type="checkbox" name="title" value="${c.value}" checked> ${c.value}
    </label>`).join('');

  document.getElementById('run-locations').innerHTML = locations.map(c => `
    <label class="run-check">
      <input type="checkbox" name="location" value="${c.value}" checked> ${c.value}
    </label>`).join('');
