...
class TestBuildSystemPrompt:
    ...
    def test_required_criteria_in_prompt(self):
        prompt = build_system_prompt({"required": ["AWS certified", "5+ years"]}, [], [])
    ...
    def test_rejected_rules_in_prompt(self):
        prompt = build_system_prompt({"rejected": ["Must be UK resident"]}, [], [])
        assert "Must be UK resident" in prompt

    def test_json_schema_fields_present(self):
        prompt = build_system_prompt({}, [], [])
        for field in ('"score"', '"score_reason"', '"dealbreakers_found"',
